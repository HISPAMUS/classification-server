...
class ModelManager:
    ...
    def eraseModel(self, id):
        ...
        if modelType is not None:
            ...
            if modelType == 'eAgnosticEnd2End':
                modelType = "end-to-end"
            else:
                modelType = "symbol-classification"
            
            modelpath = "model/" + modelType + "/" + id
            
            self.logger.info("Removing: " + modelpath)
            #Now we remove the directory first
        ...
        return (modelType is not None) #I return the condition because if it does not exist, we won't do anything
